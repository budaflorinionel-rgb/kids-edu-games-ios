...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        KidsEduGames.webView.frame = calcWebviewFrame(webviewView: webviewView, toolbarView: nil)
    }
    
    @objc func keyboardWillHide(_ notification: NSNotification) {
        KidsEduGames.webView.setNeedsLayout()
    }
    
    func initWebView() {
        KidsEduGames.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(KidsEduGames.webView);
        
        KidsEduGames.webView.uiDelegate = self;
        
        KidsEduGames.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)

        if(pullToRefresh){
            let refreshControl = UIRefreshControl()
            refreshControl.addTarget(self, action: #selector(refreshWebView(_:)), for: UIControl.Event.valueChanged)
            KidsEduGames.webView.scrollView.addSubview(refreshControl)
            KidsEduGames.webView.scrollView.bounces = true
        }

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = KidsEduGames.webView.observe(\.themeColor) { [unowned self] webView, _ in
                let backgroundColor = KidsEduGames.webView.underPageBackgroundColor;
                let themeColor = KidsEduGames.webView.themeColor;
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !KidsEduGames.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl(cachePolicy: NSURLRequest.CachePolicy = .useProtocolCachePolicy) {
        KidsEduGames.webView.load(URLRequest(url: SceneDelegate.universalLinkToLaunch ?? SceneDelegate.shortcutLinkToLaunch ?? rootUrl, cachePolicy: cachePolicy))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            KidsEduGames.webView.isHidden = false
            ...
            // Inject IAP status into webview
            if #available(iOS 15.0, *) {
                IAPBridge.shared.injectIAPStatus(webView: KidsEduGames.webView)
            }
        }
    }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                KidsEduGames.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(KidsEduGames.webView.estimatedProgress);
    ...
    deinit {
        KidsEduGames.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: KidsEduGames.webView)
        }
        ...
        // IAP message handlers
        if message.name == "iap-purchase" || message.name == "iap-restore" || message.name == "iap-status" {
            if #available(iOS 15.0, *) {
                IAPBridge.shared.handleMessage(name: message.name, webView: KidsEduGames.webView)
            }
        }
  }
}