...
func handleSubscribeTouch(message: WKScriptMessage) {
    // Push notification topic subscribe/unsubscribe (Firebase removed)
    let subscribeMessages = parseSubscribeMessage(message: message)
    if (subscribeMessages.count > 0){
        let _message = subscribeMessages[0]
        print("Topic subscribe/unsubscribe requested: \(_message.topic), unsubscribe: \(_message.unsubscribe)")
    }
}
...
func returnPermissionResult(isGranted: Bool){
    DispatchQueue.main.async(execute: {
        if (isGranted){
            KidsEduGames.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('push-permission-request', { detail: 'granted' }))")
        }
        else {
            KidsEduGames.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('push-permission-request', { detail: 'denied' }))")
        }
    })
}
func returnPermissionState(state: String){
    DispatchQueue.main.async(execute: {
        KidsEduGames.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('push-permission-state', { detail: '\(state)' }))")
    })
}
...
func checkViewAndEvaluate(event: String, detail: String) {
    if (!KidsEduGames.webView.isHidden && !KidsEduGames.webView.isLoading ) {
        DispatchQueue.main.async(execute: {
            KidsEduGames.webView.evaluateJavaScript("this.dispatchEvent(new CustomEvent('\(event)', { detail: \(detail) }))")
        })
    }
    else {
...
func handleFCMToken(){
    // Firebase removed - push token handled via APNs in AppDelegate
    print("handleFCMToken called but Firebase is removed")
}
